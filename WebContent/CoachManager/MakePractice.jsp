...
<body><div id="page">
		...
		<%Practiceaddress pa=placeService.get(studentid);%>
		<%if(pa!=null){ %>	
		<%request.getRequestDispatcher("/CoachManager/CoachManager.jsp").forward(request, response);	 %>
		<%}else{ %>
		<%for(Paddress paddress : list ){ %>
		<tr>
		<td><%=paddress.getAddress() %></td>
		<td><%=paddress.getTime() %></td>
		<td><a href="${pageContext.request.contextPath }/SelectPracticePlaceServlet?address=<%=paddress.getAddress() %>&time=<%=paddress.getTime() %>&studentid=<%=studentid %>">选择</a></td>
		</tr>
		<%}
		}%>
		</table>
